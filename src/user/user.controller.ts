...
export class UserController {
  ...
  @ApiResponse({ status: 200, description: 'OTP sent result' })
  @ApiResponse({ status: 400, description: 'Bad request - invalid email' })
  async sendEmailOtpBeforeRegistration(@Body('email') email: string): Promise<OtpResponseDto> {
    return this.userService.sendOtpEmailBeforeRegistration(email);
  }
  ...
  async verifyEmailOtpBeforeRegistration(
    @Body() verifyDto: VerifyOtpDto,
  ): Promise<OtpResponseDto> {
    return this.userService.verifyEmailBeforeRegistration(verifyDto);
  }
